...
struct FStarterWeaponData
{
	...
	/** Data table reference for attachments #1#
	UPROPERTY(EditDefaultsOnly, Category = "Data Table")
	UDataTable* AttachmentsDataTable;

	/** Local weapon data struct to keep track of ammo amounts and weapon health #1#
	UPROPERTY()
	FRuntimeWeaponData DataStruct;

	/** The array of attachments to spawn (usually inherited, can be set by instance) #1#
	UPROPERTY(EditDefaultsOnly, Category = "Data Table")
...
public:	
	/** Sets default values for this component's properties #1#
	UInventoryComponent();
	...
	/** Returns the number of weapon slots #1#
	UFUNCTION(BlueprintCallable, Category = "Inventory Component")
	int GetNumberOfWeaponSlots() const { return NumberOfWeaponSlots; }

	/** Returns the currently equipped weapon slot #1#
	UFUNCTION(BlueprintCallable, Category = "Inventory Component")
	int GetCurrentWeaponSlot() const { return CurrentWeaponSlot; }

	/** Returns the map of currently equipped weapons #1#
	UFUNCTION(BlueprintCallable, Category = "Inventory Component")
	...
	/** Returns the current weapon equipped by the player #1#
	UFUNCTION(BlueprintCallable, Category = "Inventory Component")
	AWeaponBase* GetCurrentWeapon() const {return CurrentWeapon; }

	/**  Returns the amount of ammunition currently loaded into the weapon #1#
	UFUNCTION(BlueprintCallable, Category = "Inventory Component")
	...
	/** Returns the amount of ammunition remaining for the current weapon #1#
	UFUNCTION(BlueprintCallable, Category = "Inventory Component")
	...
	/** The input actions implemented by this component #1#
	UPROPERTY()
...
private:
	...
	/** Swaps to the weapon in CurrentWeaponSlot #1#

	/**	Template function for SwapWeapon (used with the enhanced input component) #1#
	template <int SlotID>
	void SwapWeapon() { SwapWeapon(SlotID); }
	
	/** Swaps between weapons using the scroll wheel #1#
	void ScrollWeapon(const FInputActionValue& Value);

	/** Fires the weapon #1#
	void StartFire();

	/** Stops firing the weapon #1#
	void StopFire();

	/** Reloads the weapon #1#
	void Reload();

	/** Plays an inspect animation on the weapon #1#
	void Inspect();
	...
	/** Whether to print debug statements to the screen #1#
	UPROPERTY(EditDefaultsOnly, Category = "Debug")
	bool bDrawDebug = false;

	/** The distance at which pickups for old weapons spawn during a weapon swap #1#
	UPROPERTY(EditDefaultsOnly, Category = "Camera | Interaction")
	float WeaponSpawnDistance = 100.0f;

	/** THe Number of slots for Weapons that this player has #1#
	UPROPERTY(EditDefaultsOnly, Category = "Weapons | Inventory")
	int NumberOfWeaponSlots = 2;

	/** An array of starter weapons. Only weapons within the range of NumberOfWeaponSlots will be spawned #1#
	UPROPERTY(EditDefaultsOnly, Category = "Weapons | Inventory")
	...
	/** How many times we should retry the reload before cancelling it. Set to 0 for unlimited. #1#
	UPROPERTY(EditDefaultsOnly, Category = "Weapons | Behaviour")
	...
	/** The amount of times we have re-tried the reload animation #1#
	int RetryAmount; 
	...
	/** The integer that keeps track of which weapon slot ID is currently active #1#
	int CurrentWeaponSlot;

	/** The integer that keeps track of which weapon slot ID we are aiming to switch to while waiting for the unequip animation to play #1#
	int TargetWeaponSlot;

	bool bPerformingWeaponSwap;

	/** A Map storing the player's current weapons and the slot that they correspond to #1#
	UPROPERTY()
	TMap<int, AWeaponBase*> EquippedWeapons;

	/** The player's currently equipped weapon #1#
	UPROPERTY()
